...
import { useErrorLogStoreWithOut } from '@/store/modules/errorLogModules';
...
/**
 * Configure global error request handling
 */
function registerErrorRequestHandler() {
   window.addEventListener(
      'requestError',
      function (event: Event) {
         ...
         errorLogStore.addErrorLogInfo({
            ...
            detail: JSON.stringify({
               originURL: target.location?.href,
            }),
            url: (event as CustomEvent).detail?.targetURL,
            stack: 'request is not found',
